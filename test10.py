...
def generate_test_case(num_nodes, distance_range=(1, 50), weight_range=(1, 10)):
    ...
    weights = [0]  # Depot
    weights += [random.randint(weight_range[0], weight_range[1]) for _ in range$

    return distance_matrix, weights
def save_test_case_to_file(distance_matrix, weights, filename="test_case_10_nod$
...
# Parameters
num_nodes = 10  # 10 nodes (including depot)
distance_range = (1, 50)  # Random distances between 1 and 50
weight_range = (1, 10)    # Random weights between 1 and 10

# Generate test case
distance_matrix, weights = generate_test_case(num_nodes, distance_range, weight$

# Save test case to file
save_test_case_to_file(distance_matrix, weights)
print("Test case saved to 'test_case_10_nodes.txt'.")